  ...
  describe '.create' do
    ...
    context 'when the ActiveRecord major version is greater than 4' do
      ...
      context 'when ApplicationRecord is defined' do
        before do
          class ApplicationRecord < ActiveRecord::Base; end unless defined?(ApplicationRecord)
        end
      ...
      context 'when ApplicationRecord is not defined' do
        before do
          Object.send(:remove_const, :ApplicationRecord) if defined?(ApplicationRecord)
        end
    ...
    describe '.teardown' do
      it 'undefines the models' do
        ...
        expect(connection.table_exists?(:users)).to be_falsey
